...
<p>Switch to the <strong>Data</strong> tab after uploading to confirm the table looks correct. The app shows original string values for categorical columns so you can verify the mapping.</p>
  ...
  'distributions': `<html><head></head><body><h1 id="distributions">Distributions</h1>
<p>FANGS supports the following distributions for stochastic nodes in your model.</p>
<h2 id="continuous-distributions">Continuous distributions</h2>
<h3 id="dnormmean-sd-normal"><code>dnorm(mean, sd)</code> — Normal</h3>
<p><span class="math display"><em>y</em> ∼ Normal(<em>μ</em>, <em>σ</em>)</span></p>
<ul>
<li><strong>mean</strong> <span class="math inline"><em>μ</em></span>: location parameter</li>
<li><strong>sd</strong> <span class="math inline"><em>σ</em> &gt; 0</span>: standard deviation</li>
</ul>
<blockquote>
<p><strong>FANGS uses standard deviation</strong>, not precision. This differs from JAGS, which parameterises <code>dnorm</code> by precision <span class="math inline"><em>τ</em> = 1/<em>σ</em><sup>2</sup></span>. NIMBLE parameterises by precision as its default, but can also support <code>sd</code> parameterisation e.g.&nbsp;with <code>dnorm(mean, sd = sd)</code>. If converting a model from NIMBLE/JAGS, convert: <span class="math inline">$\\sigma = 1/\\sqrt{\\tau}$</span>.</p>
</blockquote>
<hr>
<h3 id="dlnormmeanlog-sdlog-log-normal"><code>dlnorm(meanlog, sdlog)</code> — Log-normal</h3>
<p><span class="math display"><em>y</em> ∼ LogNormal(<em>μ</em><sub>log</sub>, <em>σ</em><sub>log</sub>)</span></p>
<ul>
<li><strong>meanlog</strong> <span class="math inline"><em>μ</em><sub>log</sub></span>: mean of <span class="math inline">log (<em>y</em>)</span></li>
<li><strong>sdlog</strong> <span class="math inline"><em>σ</em><sub>log</sub> &gt; 0</span>: standard deviation of <span class="math inline">log (<em>y</em>)</span></li>
</ul>
<p>Use for positive, right-skewed responses.</p>
<hr>
<h3 id="dgammashape-rate-gamma"><code>dgamma(shape, rate)</code> — Gamma</h3>
<p><span class="math display"><em>y</em> ∼ Gamma(<em>α</em>, <em>β</em>)</span></p>
<ul>
<li><strong>shape</strong> <span class="math inline"><em>α</em> &gt; 0</span></li>
<li><strong>rate</strong> <span class="math inline"><em>β</em> &gt; 0</span> (rate = 1/scale)</li>
</ul>
<p>Often used as a prior for precision or rate parameters.</p>
<hr>
<h3 id="dbetaa-b-beta"><code>dbeta(a, b)</code> — Beta</h3>
<p><span class="math display"><em>y</em> ∼ Beta(<em>a</em>, <em>b</em>)</span></p>
<ul>
<li><strong>a</strong> &gt; 0, <strong>b</strong> &gt; 0 (shape parameters)</li>
</ul>
<p>Defined on <span class="math inline">(0, 1)</span>; natural prior for probabilities.</p>
<hr>
<h3 id="duniflower-upper-uniform"><code>dunif(lower, upper)</code> — Uniform</h3>
<p><span class="math display"><em>y</em> ∼ Uniform(<em>L</em>, <em>U</em>)</span></p>
<ul>
<li><strong>lower</strong> <span class="math inline"><em>L</em></span>, <strong>upper</strong> <span class="math inline"><em>U</em></span>: endpoints of the interval</li>
</ul>
<p>A common non-informative prior for variance or scale parameters (e.g.&nbsp;<code>sigma ~ dunif(0, 100)</code>).</p>
<hr>
<h3 id="dexprate-exponential"><code>dexp(rate)</code> — Exponential</h3>
<p><span class="math display"><em>y</em> ∼ Exponential(<em>λ</em>)</span></p>
<ul>
<li><strong>rate</strong> <span class="math inline"><em>λ</em> &gt; 0</span></li>
</ul>
<p>Equivalent to <code>dgamma(1, rate)</code>. Useful as a weakly informative prior for scale parameters.</p>
<hr>
<h2 id="discrete-distributions">Discrete distributions</h2>
<h3 id="dpoislambda-poisson"><code>dpois(lambda)</code> — Poisson</h3>
<p><span class="math display"><em>y</em> ∼ Poisson(<em>λ</em>)</span></p>
<ul>
<li><strong>lambda</strong> <span class="math inline"><em>λ</em> &gt; 0</span>: expected count</li>
</ul>
<p>Standard likelihood for count data. Combine with a log link: <code>log(mu[i]) &lt;- alpha + beta * x[i]</code>.</p>
<hr>
<h3 id="dbernprob-bernoulli"><code>dbern(prob)</code> — Bernoulli</h3>
<p><span class="math display"><em>y</em> ∼ Bernoulli(<em>p</em>)</span></p>
<ul>
<li><strong>prob</strong> <span class="math inline"><em>p</em> ∈ (0, 1)</span>: probability of success</li>
</ul>
<p>For binary (0/1) outcomes. Use with a logit link: <code>logit(p[i]) &lt;- alpha + beta * x[i]</code>.</p>
<hr>
<h3 id="dbinprob-size-binomial"><code>dbin(prob, size)</code> — Binomial</h3>
<p><span class="math display"><em>y</em> ∼ Binomial(<em>n</em>, <em>p</em>)</span></p>
<ul>
<li><strong>prob</strong> <span class="math inline"><em>p</em> ∈ (0, 1)</span>: success probability</li>
<li><strong>size</strong> <span class="math inline"><em>n</em></span>: number of trials</li>
</ul>
<p>Generalises Bernoulli to multiple trials. Also used with a logit link.</p>
</body></html>`,
...
<h2 id="improving-sampling">Improving sampling</h2>
<p>MCMC chains for mixed effects models often have slow convergence due to the hierarchical structure. The means for each group can be highly correlated with the global mean (to see this look at the ‘Joint’ tab after you fit a model). To improve sampling, you can update the model code to use a <strong>non-centered parameterisation</strong>:</p>
<pre><code>b[j] ~ dnorm(0, 1)  # standard normal
b_scaled[j] &lt;- b[j] * sigma.b  # scale by the hyperparameter
mu[i] &lt;- alpha + beta * x[i] + b_scaled[group[i]]</code></pre>
<p>removing the direct dependence of b[j] on sigma.b. This reduces the correlation between parameters and can lead to much faster convergence.</p>
...
<h2 id="different-from-bugsjagsnimble">Different from BUGS/JAGS/NIMBLE</h2>
...
<ol type="1">
<li><p><strong>Slow mixing</strong>: The slope (<span class="math inline"><em>β</em></span>) and intercept (<span class="math inline"><em>α</em></span>) become strongly correlated in the posterior. Component-wise samplers like Gibbs struggle with correlated parameters, producing chains that mix slowly and need many more samples to converge.</p></li>
<li><p><strong>Numerical conditioning</strong>: The conjugate update formula involves summing squared predictor values (<span class="math inline">∑<em>x</em><sub><em>i</em></sub><sup>2</sup></span>). For large-scale predictors this sum can become very large, potentially causing numerical instability.</p></li>
</ol>
...
<p>Each continuous predictor <span class="math inline"><em>x</em></span> is transformed to:</p>
<p><span class="math display">$$x_{\\text{scaled}} = \\frac{x - \\bar{x}}{\\text{SD}(x)}$$</span></p>
<p>This gives <span class="math inline"><em>x</em><sub>scaled</sub></span> a mean of 0 and a standard deviation of 1. The sampler then works on this scaled version.</p>
...
<p><span class="math display">$$\\hat{\\beta}_{\\text{original}} = \\frac{\\hat{\\beta}_{\\text{scaled}}}{\\text{SD}(x)}, \\qquad \\hat{\\alpha}_{\\text{original}} = \\hat{\\alpha}_{\\text{scaled}} - \\hat{\\beta}_{\\text{scaled}} \\cdot \\frac{\\bar{x}}{\\text{SD}(x)}$$</span></p>
<p>So a slope of 5 in scaled space becomes 0.05 in original space if <span class="math inline">SD(<em>x</em>) = 100</span> — exactly the correct interpretation.</p>
<h2 id="when-does-scaling-apply">When does scaling apply?</h2>
<p>Scaling is applied only to: - Continuous numeric columns (not categorical/factor variables) - Columns with a standard deviation greater than 2 - Predictor columns (not the response variable or grouping indices)</p>
