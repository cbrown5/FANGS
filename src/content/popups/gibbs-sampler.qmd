...
- Normal likelihood + normal prior → normal full conditional (updates α, β and random effects)
- Beta prior + Bernoulli/Binomial likelihood → beta full conditional
- Gamma prior + Poisson likelihood → gamma full conditional

## Slice sampling fallback

When conjugacy does not apply, FANGS falls back to **slice sampling** — a derivative-free method that still samples from the correct distribution, just less efficiently. This is used for logistic regression and for the standard-deviation parameter `sigma` (the normal SD has no conjugate prior under FANGS's SD parameterisation).
