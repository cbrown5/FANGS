...
<p>Answer the two diagnostic questions below.</p>`,
  'm11-identifiability-priors': `<h1>Identifiability &amp; bad ESS/R-hat: a case study</h1>
...
<h2>A two-intercept jaw-length model</h2>
<p>Recall the jaw-length regression:</p>
<div class="math display">\\[\\text{Lower\\_jaw\\_length}_i = \\alpha + \\beta \\times \\text{Standard\\_length}_i + \\varepsilon_i\\]</div>
...
<h2>Step 1 — watch the chains fail</h2>
<ol>
<li>Load <code>fish-lengths.csv</code> in FANGS, setting <code>y = Lower_jaw_length</code> and</li>
</ol>
<p>   <code>x = Standard_length</code>.</p>
<ol>
...
<p>::: callout-tip The data constrain a <em>line</em> through <span class="math inline">\\((\\alpha, \\alpha_2)\\)</span> space, not a <em>point</em>. The sampler slides freely along that line — so it does. :::</p>
<h2>Step 2 — see the bimodality</h2>
...
<h2>Step 3 - check the joint distribution</h2>
...
<h2>Step 4 — informed priors to the rescue</h2>
...
<table>
<thead><tr><th>Step</th><th>Prior on <code>alpha2</code></th><th>What it says</th></tr></thead>
<tbody>
<tr><td>1</td><td><code>alpha2 ~ dnorm(0, 100)</code></td><td>&quot;alpha2 could be almost anything&quot;</td></tr>
<tr><td>2</td><td><code>alpha2 ~ dnorm(10, 20)</code></td><td>&quot;alpha2 is centered on 10, but could be much higher or lower&quot;</td></tr>
<tr><td>3</td><td><code>alpha2 ~ dnorm(10, 2)</code></td><td>&quot;alpha2 is very close to 10&quot;</td></tr>
</tbody>
</table>
<h2>Record your runs</h2>
<p>Fit the two-intercept jaw-length model under <strong>at least the three priors on <code>alpha2</code></strong> in the table at. For each run, record the prior SD, the worst R-hat of <code>alpha</code>/<code>alpha2</code>, the minimum ESS, and a quick note on the posterior shape (<em>ridge / bimodal / single peak</em>). Add your records in the app at the end of this module. Two or more rows will unlock the module.</p>
...
<h2>Coding a two-level factor in FANGS</h2>
<p>FANGS encodes a text column like <code>treatment</code> into integers automatically, using the order of first appearance in the CSV. For <code>clownfish-oa.csv</code> this gives:</p>
<table>
<thead><tr><th>Level</th><th>Code</th></tr></thead>
<tbody>
<tr><td>Normal seawater</td><td>1</td></tr>
<tr><td>Acidified seawater</td><td>2</td></tr>
</tbody>
</table>
<p>To build the familiar <strong>contrast</strong> parameterisation (control mean + effect of treatment), subtract 1 from the code so the control maps to 0:</p>
<div class="math display">\\[\\mu_i = \\alpha + \\beta_{\\text{trt}} \\times (\\text{treatment}_i - 1)\\]</div>
<ul>
<li><span class="math inline">\\(\\alpha\\)</span> — the <strong>control mean</strong> (Normal seawater, when <code>treatment = 1</code>).</li>
<li><span class="math inline">\\(\\beta_{\\text{trt}}\\)</span> — the <strong>acidification effect</strong>: how much the mean shifts for acidified fish.</li>
</ul>
<p>If <span class="math inline">\\(\\beta_{\\text{trt}}\\)</span>'s 95% CI clearly excludes 0, acidification credibly shifts the response.</p>
<h2>The model</h2>
<pre><code>model {
  for (i in 1:N) {
...
<h2>Your task</h2>
<ol>
<li>Load <code>clownfish-oa.csv</code> into FANGS. Set <code>y = time</code> and <code>x = treatment</code>.</li>
<li>Copy the model above into the editor and run it (4 chains, 2000 samples).</li>
<li>Inspect the <strong>Trace</strong> and <strong>Summary</strong> tabs to check convergence (R-hat ≈ 1, ESS &gt; 400).</li>
<li>Read the posterior mean and 95% CI for <code>alpha</code> and <code>beta_trt</code> from the Summary tab.</li>
<li>Enter those values in the challenge below to complete the module.</li>
</ol>
<p>::: callout-tip Look at the <strong>Predictions</strong> tab after fitting. The two groups should stand out clearly, and the model's posterior mean and 95% CI for each group will be overlaid on the data. :::</p>
<h2>Interpreting the result</h2>
<p>Once the model has converged, ask yourself:</p>
<ul>
<li>Does the 95% CI for <code>beta_trt</code> include 0? If not, there is strong evidence that acidification changes behaviour.</li>
...
<p>Fit the logistic model to <code>presence.csv</code> and check your posterior means for <span class="math inline">\\(\\alpha\\)</span> and <span class="math inline">\\(\\beta\\)</span> (logit scale).</p>`,
  'm17-binomial-three-level': `<h1>Binomial with a 3-level factor</h1>
...
<p>Answer the concept quiz below before we fit one.</p>`,
  'm19-random-effects-fit': `<h1>Fit a random-effects model</h1>
